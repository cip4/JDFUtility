...
public class ComplexTypeList extends JPanel implements ListSelectionListener, MouseListener, ActionListener
{
	...
	public ComplexTypeList(final GeneratorUI mother)
	{
		motherComp = mother;
		init();
	}
	...
	private void init()
	{
		...
		{
			final String strElementName = (String) defaultlistModel.get(i);
		...
		final JScrollPane jsc = new JScrollPane(myList);
	...
	public void mouseClicked(final MouseEvent e)
	{
		...
		if (i % 2 == 0)
		{
			final int j = myList.locationToIndex(new Point(e.getX(), e.getY()));
			fireListDoubleClickedEvent((String) listModel.get(j));
		}
	}

	public void addListDoubleClickedListener(final ListDoubleClickedListener aListener)
	{
		doubleClickedListener.add(aListener);
	}

	public void removeListDoubleClickedListener(final ListDoubleClickedListener aListener)
	{
		doubleClickedListener.remove(aListener);
	}

	protected void fireListDoubleClickedEvent(final String strName)
	{
		for (int i = 0; i < doubleClickedListener.size(); i++)
		{
			final ListDoubleClickedEvent event = new ListDoubleClickedEvent(this, strName);
	...
	public void actionPerformed(final ActionEvent e)
	{
		if (e.getSource() == popup_load)
		{
			...
			{
				final File toGenerateFile = fc.getSelectedFile();
		...
		else if (e.getSource() == popup_safe)
		{
			...
			if (fc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION)
			{
				...
				try
				{
					final StringBuffer strBuff = new StringBuffer(500);
					...
					final DefaultTableModel dtm = motherComp.getStatusPanel().getDefaultTableModel();
					dtm.insertRow(0, new Object[] { "Writing " + toGenerateFile.getName(), "Working..." });
		...
		{
			final int[] selected = myList.getSelectedIndices();
		...
		{
			final AddAutoFileDialog addAuto = new AddAutoFileDialog();
			addAuto.pack();
	...
	public void loadFileList(final File toGenerateFile)
	{
		if (toGenerateFile.canRead())
		{
			...
			try
			{
				...
				while ((strLineOfFile = buffreader.readLine()) != null)
				{
					if (!strLineOfFile.startsWith("JDFAuto") || !strLineOfFile.endsWith(".java"))
					{
						JOptionPane.showMessageDialog(this, strLineOfFile + lineSep + "is a malformed filename." + lineSep
								+ "Filenames must start with \"JDFAuto\" and end with \".java\"" + lineSep + lineSep + " Cant read file", "Malformed Filename in User-filelist",
				...
				{
					final String strElementName = (String) loadedListModel.get(i);
					listModel.addElement(strElementName);
				}
				validate();
			}
	...
	static
	{
		...
		defaultlistModel.addElement("JDFAutoAuthenticationCmdParams.java");
		defaultlistModel.addElement("JDFAutoAuthenticationQuParams.java");
		defaultlistModel.addElement("JDFAutoAuthenticationResp.java");
		...
		defaultlistModel.addElement("JDFAutoDieLayoutProductionParams.java");
		...
		defaultlistModel.addElement("JDFAutoMarkActivation.java");
		...
		defaultlistModel.addElement("JDFAutoMetadataMap.java");
		defaultlistModel.addElement("JDFAutoMilestone.java");
		...
		defaultlistModel.addElement("JDFAutoPositionObj.java");
		...
		defaultlistModel.addElement("JDFAutoRefAnchor.java");
		defaultlistModel.addElement("JDFAutoReferenceXObjParams.java");
		...
		defaultlistModel.addElement("JDFAutoRepeatDesc.java");
		...
		defaultlistModel.addElement("JDFAutoRuleLength.java");
		...
		defaultlistModel.addElement("JDFAutoSubscriptionFilter.java");
		defaultlistModel.addElement("JDFAutoSubscriptionInfo.java");
		defaultlistModel.addElement("JDFAutoSurfaceMark.java");
		defaultlistModel.addElement("JDFAutoSystemTimeSet.java");
		defaultlistModel.addElement("JDFAutoTabDimensions.java");
		...
		defaultlistModel.addElement("JDFAutoVarnishingParams.java");
