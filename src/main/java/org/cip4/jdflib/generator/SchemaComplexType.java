...
public class SchemaComplexType implements Serializable
{
	...
	private void setStrExtendsName()
	{
		...
		{
			if ("Preview".equals(m_SchemaComplexTypeName) || "Address".equals(m_SchemaComplexTypeName) || "ComChannel".equals(m_SchemaComplexTypeName)
					|| "Person".equals(m_SchemaComplexTypeName))
				strExtendsName = "JDFResource";
			else
				strExtendsName = "JDFElement";
		}
	...
	private void setStrRequiredAttributes()
	{
		for (int i = 0; i < m_vSchemaAttributes.size(); i++)
		{
			...
			if ("optional".equals(nSchemaAttribute.getStrUse()) || !JDFConstants.EMPTYSTRING.equals(nSchemaAttribute.getStrDefault())
